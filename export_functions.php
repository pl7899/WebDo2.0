...
$closingText = "</body> </html>";
	...
	if($weekValue == "0" || $weekValue == "1" || $weekValue == "t" || $weekValue == "T")
	{
		...
		fwrite($myfile, "document.getElementById('weekInformation').innerHTML = \" " . "week : " . date("W", strtotime($nextDay)) . " , " . date("M dS", strtotime($nextDay)) . " - " . date("dS", strtotime($nextDay . "+ 6 days")) . " \" \n");
		...
		if($row = mysqli_fetch_array($rows)) 
		{
			fwrite($myfile, "document.getElementById('saturdayLineSix').innerHTML = \" " . substr($row['taskDescription'], 0, $weekendCharacters) . " \" \n");
		} 
		// SUNDAY
	    $nextDay = date("Y-m-d", strtotime($nextDay."+ 1 days"));
		$findWeeksTasks = "SELECT *  FROM `todoActions` WHERE `isOpen`=\"1\" AND `targetDate`=\"" . $nextDay . "\"  AND NOT `project`=\"work\" ORDER BY `priority` ASC";
		...
		//JJ Task LList
		$findWeeksTasks = "SELECT *  FROM `todoActions` WHERE `isOpen`=\"1\" AND `project`=\"jenny Stuff\"  AND NOT `project`=\"work\" ORDER BY `priority` ASC";
		$rows = mysqli_query($db, $findWeeksTasks);
		fwrite($myfile, "document.getElementById('sundayDate').innerHTML = \" " . date("M dS", strtotime($nextDay)) . " \" \n");
